...
After `go mod init`, edit `lsp/go.mod` and set the `go` directive to `go 1.24` so the module's required Go version matches what the release workflow (Task 11) installs. Note: d2 v0.7.1 requires Go 1.24.
